...
오늘은 R에서 파이썬을 설치해서 사용하는 방법에 대해 배우겠다. 만약 파이썬이 없으면 [여기](https://www.python.org)에 들어가 다운로드 한 후에 설치한다. 그리고 파이썬 모듈을 설치하기 위해 먼저 가상환경을 설치해야된다.
...
```{r load_required, message=FALSE, echo=FALSE}
...
파이썬을 사용할때 R Console 창에 miniconda를 설치할꺼라고 나오면 `Y`를 눌러 아나콘다 환경을 설치한다. 기본으로 `reticulate`을 실행시키면 `r-reticulate`이라는 가상환경을 사용한다. 기본으로 `reticulate`은 `Sys.which("python")`에 설치된 파이썬을 사용한다.
...
```{r config_conda, message=FALSE, eval=FALSE}
